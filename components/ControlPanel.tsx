...
export const ControlPanel: React.FC<ControlPanelProps> = ({ 
  ...
  return (
    <div className="bg-slate-900 border-t border-slate-800 p-4 flex flex-col md:flex-row items-center justify-between shadow-2xl gap-4">
      {/* Connection Controls */}
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-lg" title="Current connection status to the AI service.">
      ...
      <div className="flex items-center gap-4 bg-slate-800/50 p-2 rounded-xl">
        ...
        <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${autoRead ? 'bg-amber-600 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}`}
        ...
        <button 
            ...
            className={`p-2 rounded-full transition-all ${
                !hasVerse ? 'opacity-30 cursor-not-allowed text-slate-500' :
                isSpeaking ? 'bg-rose-500/20 text-rose-500 hover:bg-rose-500/30' : 'bg-slate-700 text-slate-200 hover:bg-slate-600'
            }`}
            title={isSpeaking ? "Stop the current audio reading." : "Read the currently displayed verse aloud."}
        >
            {isSpeaking ? (
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                    <path fillRule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.348c1.295.712 1.295 2.573 0 3.285L7.28 19.991c-1.25.687-2.779-.217-2.779-1.643V5.653z" clipRule="evenodd" />
                    <rect x="6" y="6" width="12" height="12" rx="2" fill="currentColor"/> 
                    {/* Actually, square is stop. Let's make it a square. */}
