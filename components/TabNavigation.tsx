...
export const TabNavigation: React.FC<TabNavigationProps> = ({ currentMode, onModeChange }) => {
  ...
  return (
    <div className="flex justify-center gap-1 p-1 bg-white/50 dark:bg-slate-900/50 backdrop-blur rounded-full border border-slate-200/50 dark:border-slate-800/50 overflow-x-auto max-w-[90vw] transition-colors">
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 whitespace-nowrap ${
              isActive
                ? 'bg-amber-500 text-white dark:text-slate-950 shadow-lg shadow-amber-500/20'
